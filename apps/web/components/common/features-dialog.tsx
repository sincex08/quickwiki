 ...
 * 最近更新：2026-09-19（手机列表在笔记本下同屏展示「子笔记本」与「笔记」两块，点子笔记本即切换位置；
 * 手机新建笔记按钮抬高到统计栏之上，不再压住「仅标题」；左栏笔记行加文件图标、笔记本行保持色点，
 * 嵌套时两类条目一眼可辨；子笔记本与笔记并存时各带小标题与分隔，层级更清晰）
 * 上一版 2026-09-18：手机端打开笔记默认「预览」视图、桌面端默认「编辑」；「预览点击编辑」开关默认关闭；
 * 预览视图里任务清单可直接勾选 / 取消；置顶改为**永远排最前**——此前手动排过顺序的容器里置顶不决定位置；
 * 未手动排序的笔记改按**创建时间升序**排列，早创建的在前——此前按最后修改时间，编辑笔记会把它推到最前；
 * 修复手机长按拖动：长按不再选中标题文字、复制菜单不再盖住拖动
 * 上一版 2026-09-17：搜索改为顶部悬浮结果面板：不再替换左栏树，结果含笔记本归属、关键字高亮与上下文摘要；
...
const FEATURE_GROUPS: FeatureGroup[] = [
  {
    ...
    title: "笔记管理",
    items: [
      "新建 / 编辑 / 删除（二次确认）/ 置顶（永远排最前，手动排过顺序也一样）",
      ...
      "列表：置顶永远最前 + 创建时间升序、加载更多分页；手机左滑卡片置顶 / 删除",
      "手机列表会把当前笔记本下的子笔记本一并列出（带颜色与笔记数），点一下即进入该子笔记本",
  ...
  },
  {
    ...
    items: [
      ...
      "混合预览：渲染视图点击任意块直接编辑该块源码（失焦或 Ctrl+Enter 提交、Esc 取消），文末可追加块；编辑栏「点击编辑」开关可切为纯只读（默认关闭）",
      "手机端打开笔记默认进入「预览」（读优先），桌面端默认「编辑」；三模式随时可切",
      "预览视图里任务清单可直接勾选 / 取消勾选，无需切到编辑模式",
  ...
  },
  {
    ...
    items: [
      ...
      "任务清单：编辑视图可勾选、Tab 嵌套；预览视图也可直接点击勾选 / 取消",
